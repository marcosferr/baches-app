...
export async function POST(request: Request) {
  try {
    ...
    if (rateLimited) {
      return NextResponse.json({ error: "Too many comments posted. Please try again later." }, { status: 429 })
    }
    ...
    if (report.authorId !== session.user.id) {
      await createNotification({
        ...
        message: `${session.user.name} comentó en tu reporte: "${text.length > 50 ? text.substring(0, 50) + "..." : text}"`,
        type: "COMMENT",
